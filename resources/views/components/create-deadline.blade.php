<form class="max-w-md mx-auto" action={{ route('deadlines.store') }} method="POST">
    @csrf
    <div class="relative z-0 w-full mb-8 group">
        <input type="text" name="deadline_name" id="deadline_name"
            class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
            placeholder=" " required />
        <label for="deadline_name"
            class="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Deadline
            Name</label>
    </div>
    <div class="relative z-0 w-full mb-8 group">
        <input type="text" name="deadline_description" id="deadline_description"
            class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
            placeholder=" " />
        <label for="deadline_description"
    ...
    <div class="relative z-0 w-full mb-8 group">
        <input type="date" name="deadline_date" id="deadline_date" min="{{ now()->toDateString() }}"
            class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
            placeholder=" " required />
    ...
    <div class="border-t border-b border-blue-600 p-5 pb-0 mb-4">
        ...
        @foreach (App\Models\ClientTypes::all() as $type)
            <div class="flex items-center mb-4">
                <input id="{{ 'type_' . $type->id }}" name="client_types[]" type="checkbox" value="{{ $type->id }}"
